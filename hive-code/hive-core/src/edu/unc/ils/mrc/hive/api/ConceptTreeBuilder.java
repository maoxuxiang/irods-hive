...
public class ConceptTreeBuilder {
	...
	private void getTree(final ConceptNode node, final String uri) {
	...
	public void add(final SKOSConcept concept, final SKOSSearcher searcher) {
		...
		if (broaders.size() > 0) {
			for (String key : broaders.keySet()) {
				...
				SKOSConcept broader = searcher.searchConceptByURI(
						qname.getNamespaceURI(), qname.getLocalPart());

				String broaderUri = broader.getQName().getNamespaceURI()
						+ broader.getQName().getLocalPart();
	...
	public void add(final Concept concept, final SesameManager manager) {
		...
		if (broaders.size() > 0) {
			for (Concept b : broaders) {
				...
				cn.setAltLabels(new ArrayList<String>(broader
						.getSkosAltLabels()));
				// concepts.put(broaderUri, broaderLabel);
