...
public class AtomSynchronizer {
	 ...
	 * @param scheme
	 */
	public AtomSynchronizer(final SKOSScheme scheme) {
		this.scheme = scheme;
		feedUrl = scheme.getAtomFeedURL();
	}
	...
	public void processUpdates() {
		...
		try {
			...
			// configuration file.
			Date lastUpdate = (scheme.getLastUpdateDate() != null) ? scheme
					.getLastUpdateDate() : scheme.getCreationDate();
	 ...
	 * @param endDate
	 */
	public void processUpdates(final Date startDate, final Date endDate) {
	 ...
	 * @throws Exception
	 */
	public void readFeed(final Date startDate, Date endDate) throws Exception {
		...
		for (int x = 1; x < MAX_PAGES; x++) {
			...
			for (int i = 0; i < nodes.getLength(); i++) {
				...
				for (int j = 0; j < children.getLength(); j++) {
					...
					if (name != null) {
						if (name.equals("updated")) {
							...
							if (date != null) {
								updatedDate = javax.xml.bind.DatatypeConverter
				...
				if (updatedDate != null) {
					if (startDate != null && endDate != null) {
						if (updatedDate.after(startDate)
								&& updatedDate.before(endDate)) {
			...
			NodeList deletedNodes = (NodeList) xpath.evaluate(expression,
