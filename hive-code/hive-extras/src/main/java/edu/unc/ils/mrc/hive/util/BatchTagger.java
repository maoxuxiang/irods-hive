...
/**
 * Simple command line tagger. Given a directory of PDF files, generates text
 * files with keyphrases from the specified vocabulary.
 */
public class BatchTagger {
	...
	public static void main(final String[] args) {
		if (args.length != 3) {
			System.err
					.println("Usage: java "
							+ BatchTagger.class.getName()
							+ "[path to hive.properties] [path to directory] [vocabulary]");
			return;
		}
		...
		for (File file : files) {
			try {
				...
				String textFileName = pdfName.substring(0,
						pdfName.lastIndexOf('.'))
						+ ".txt";

				String keyFileName = pdfName.substring(0,
						pdfName.lastIndexOf('.'))
						+ ".key";
				FileWriter keyFileWriter = new FileWriter(keyFileName);

				List<SKOSConcept> concepts = tagger.getTags(
						file.getAbsolutePath(), vocabularies, searcher,
						NUM_TERMS, 2);
