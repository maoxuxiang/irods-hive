...
public class TGNThesaurus {
	...
	public TGNThesaurus() {
		records = new Hashtable<String, TGNRecord>();
	}
	...
	public TGNRecord getRecord(final String key) {
		return records.get(key);
	}

	public void setRecord(final TGNRecord record) {
		records.put(record.getSubjectID(), record);
	}
	...
	public void printSKOSThesaurus(final String path)
		...
		for (String s : records.keySet()) {
			...
			pr.println("\t<skos:inScheme rdf:resource=\"" + re.getURIBASE()
					+ "\"/>");
			pr.println("\t<skos:prefLabel>" + re.getPreferredTerm()
					+ "</skos:prefLabel>");
			...
			if (scopeNote.length() > 0) {
				pr.println("\t<skos:scopeNote>" + scopeNote
						+ "</skos:scopeNote>");
			}
