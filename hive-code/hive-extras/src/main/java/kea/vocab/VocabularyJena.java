...
public class VocabularyJena extends Vocabulary {
	...
	public VocabularyJena(final String vocabularyName,
		...
		if (vocabularyFormat.equals("skos")) {
			...
			if (!SKOS.exists()) {
				System.err.println("File VOCABULARIES/" + vocabularyName
						+ ".rdf does not exist.");
				System.exit(1);
			}
			useSkos = true;
			...
			if (!USE.exists()) {
				System.err.println("File VOCABULARIES/" + vocabularyName
						+ ".list.use does not exist.");
				System.exit(1);
			}
			if (!REL.exists()) {
				System.err.println("File VOCABULARIES/" + vocabularyName
						+ ".rel.p1 does not exist.");
				System.exit(1);
			}
			// if (!RT.exists()) {
			// System.err.println("File VOCABULARIES/" + vocabularyName +
			// ".pairs.p1 does not exist.");
			// System.exit(1);
			// }
	...
	public void buildSKOS() throws Exception {
		...
		try {
			...
			model.read(
					new InputStreamReader(new FileInputStream(SKOS), "UTF-8"),
					"");
			...
			while (iter.hasNext()) {
				...
				if (rel.equals("prefLabel")) {
					...
					// VocabularyENrev.get(id) + " with id (" + id + ")" +
					// " has a non-descriptor " + non_descriptor + " (" +
					// id_non_descriptor + ")");

				} else if (rel.equals("broader") || rel.equals("narrower")
						|| rel.equals("composite") || rel.equals("compositeOf")
						|| rel.equals("hasTopConcept") || rel.equals("related")) {
					...
					// VocabularyENrev.get(id) + " with id " + id +
					// " has a " + rel + " term " +
					// VocabularyENrev.get(id_related) + " with id (" +
					// id_related + ")");
			...
			// System.out.println(VocabularyREL.size() +
