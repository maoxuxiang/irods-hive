...
/**
 * Controller for query of all by related vocabulary term, queries all files and
 * collections that are annotated with a term that is related to the associated
 * term
 * 
 ...
 */
@Controller
public class QueryAllForRelatedVocabularyTerm {

	private JargonHiveQueryService jargonHiveQueryService;

	public static final Logger log = LoggerFactory
			.getLogger(QueryAllForRelatedVocabularyTerm.class);
	 ...
	 * 
	 */
	@RequestMapping(value = "/preparedQuery/allForRelatedVocabularyTerm", method = RequestMethod.GET, produces = "application/json")
	@ResponseBody
	public String queryForAllForRelatedVocabularyTerm(
			@RequestParam("vocabUri") final String vocabUri,
			@RequestParam("termId") final String termId)
			throws HiveQueryServiceException {

		log.info("queryForAllForRelatedVocabularyTerm");
		...
		try {
			return jargonHiveQueryService.queryForUriRelated(sb.toString());
